Public Class frmOrcamento
    Dim sql As String
    Dim tbOrcamento, tbOrcamento_pecas, tbOrcamento_servico, tbClientes, TbPecas, TbFuncionarios, tbServico As DataTable ' ADODB.Recordset
    ...
    Private Sub cboClientes_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles cboClientes.LostFocus
        ...
        cboClientes.Tag = tbClientes.Rows(0)("codigo")
    End Sub

    ' Repita o padrão acima para os eventos relacionados a TbFuncionarios, TbPecas, tbServico
    ...
    Private Sub btnNovo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNovo.Click
        ...
        ' Restante do código para manipular tbOrcamento
    End Sub
    ...
    Private Sub btnOkservico_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnOkservico.Click
        ...
        ' Restante do código para manipular tbOrcamento_servico
    End Sub
    'Private Sub cboClientes_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles cboClientes.GotFocus
    '    cboClientes.Items.Clear()
    '    sql = "Select * from tbclientes order by nome"
    ...
    '    sql = "Select * from tbOrcamento order by Codigo desc"
    '    tbOrcamento = OpenRecordset(sql)
    ...
    Private Sub btnConsultar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnConsultar.Click
        ...
        Dim dtOrcamento As DataTable = OpenRecordset(sql)
        If dtOrcamento.Rows.Count = 0 Then
            MsgBox("Não existe o orçamento solicitado !", MsgBoxStyle.Information)
            Exit Sub
        End If
        cboClientes.Text = dtOrcamento.Rows(0)("cliente").ToString()
        txtTotalgeral.Text = dtOrcamento.Rows(0)("totalgeral").ToString()
        txtDesconto.Text = dtOrcamento.Rows(0)("desconto").ToString()
        txtData.Text = dtOrcamento.Rows(0)("data").ToString()
        cbxAprovado.Checked = Convert.ToBoolean(dtOrcamento.Rows(0)("aprovado"))
    End Sub
    ...
    '    With Gradeservico
    '        .Rows.Clear()
    '        sql = "Select * from tbOrcamento_servico order by Codigo"
    '        tbOrcamento_servico = OpenRecordset(sql)
    '        If tbOrcamento_servico.RecordCount <> 0 Then
    ...
    '    With Gradepeca
    '        .Rows.Clear()
    '        sql = "Select * from tbOrcamento_pecas order by Codigo"
    ...
    'End Sub
    Private Sub montargradeservico()
        ' Assuming Gradeservico is a DataGridView
        With Gradeservico
            .Rows.Clear()
            sql = "Select * from tbOrcamento_servico order by Codigo"
            Dim dtOrcamento_servico As DataTable = OpenRecordset(sql)
            If dtOrcamento_servico.Rows.Count <> 0 Then
                For Each row As DataRow In dtOrcamento_servico.Rows
                    .Rows.Add(False)
    ...
    Private Sub montargradepeca()
        ' Assuming Gradepeca is a DataGridView
        With Gradepeca
            .Rows.Clear()
            sql = "Select * from tbOrcamento_pecas order by Codigo"
            Dim dtOrcamento_pecas As DataTable = OpenRecordset(sql)
            If dtOrcamento_pecas.Rows.Count <> 0 Then
                For Each row As DataRow In dtOrcamento_pecas.Rows
                    .Rows.Add(False)
                    Dim x As Integer = .Rows.Count - 1
                    .Rows(x).Cells(0).Value = row("nrorcamento").ToString()
    ...
    Private Sub btnOkpeca_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnOkpeca.Click
        ...
        sql = "Select sum(valortotal) as Valortotal from tbOrcamento_pecas where nrorcamento = " & txtNumero.Text
        Dim dtTotalPecas As DataTable = OpenRecordset(sql)
        If dtTotalPecas.Rows.Count <> 0 Then
            txtValortotalpeca.Text = FormatCurrency(dtTotalPecas.Rows(0)("valortotal"))
        Else
            txtValortotalpeca.Text = FormatCurrency(0)
        End If
    ...
    Private Sub btnExcluirservico_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnExcluirservico.Click
        ...
        Else
            ...
            txtData.Text = dtOrcamento_servico.Rows(0)("data").ToString()
            cboClientes.Text = dtOrcamento_servico.Rows(0)("cliente").ToString()
            cboFuncionario.Text = dtOrcamento_servico.Rows(0)("funcionario").ToString()
            cboServico.Text = dtOrcamento_servico.Rows(0)("servico").ToString()
            txtTotalservico.Text = dtOrcamento_servico.Rows(0)("valortotal").ToString()
    ...
    Private Sub btnExcluirpeca_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnExcluirpeca.Click
        ...
        Else
            ...
            cboPecas.Text = dtOrcamento_pecas.Rows(0)("peca").ToString()
            txtValorpeca.Text = dtOrcamento_pecas.Rows(0)("valorunitario").ToString()
            txtQuantidadepeca.Text = dtOrcamento_pecas.Rows(0)("quantidade").ToString()
            txtTotalpeca.Text = dtOrcamento_pecas.Rows(0)("valortotal").ToString()
    ...
    '    wcimagem = Image.FromFile("\Oficina Mecânica\drag.png")
    ...
    Private Sub PrintDocument1_PrintPage(ByVal sender As System.Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs) Handles PrintDocument1.PrintPage
        ...
        dt.Columns.Add("nrorcamento")
        dt.Columns.Add("data")
        dt.Columns.Add("cliente")
        dt.Columns.Add("funcionario")
        dt.Columns.Add("servico")
        dt.Columns.Add("valorservico")
        dt.Columns.Add("desconto")
        dt.Columns.Add("valortotal")
        dt.Columns.Add("pecas")
        dt.Columns.Add("valorunitariopeca")
        dt.Columns.Add("quantidade")
        dt.Columns.Add("valortotalpeca")

        ' Assuming tbOrcamento is your DataTable, you would populate dt here

        For Each row In dt.Rows
            e.Graphics.DrawString(row("nrorcamento").ToString(), MYFONT, Brushes.Black, X1, Y1)
            e.Graphics.DrawString(Microsoft.VisualBasic.Left(row("data").ToString(), 30), MYFONT, Brushes.Black, X1 + 180, Y1)
            e.Graphics.DrawString(row("cliente").ToString(), MYFONT, Brushes.Black, X1 + 300, Y1)
            e.Graphics.DrawString(row("funcionario").ToString(), MYFONT, Brushes.Black, X1 + 400, Y1)
            e.Graphics.DrawString(row("servico").ToString(), MYFONT, Brushes.Black, X1, Y1 + 40)
            e.Graphics.DrawString(Microsoft.VisualBasic.Left(row("valorservico").ToString(), 30), MYFONT, Brushes.Black, X1 + 180, Y1 + 40)
            e.Graphics.DrawString(row("desconto").ToString(), MYFONT, Brushes.Black, X1 + 300, Y1 + 40)
            e.Graphics.DrawString(row("valortotal").ToString(), MYFONT, Brushes.Black, X1 + 400, Y1 + 40)
            e.Graphics.DrawString(row("pecas").ToString(), MYFONT, Brushes.Black, X1 + 5, Y1 + 90)
            e.Graphics.DrawString(Microsoft.VisualBasic.Left(row("valorunitariopeca").ToString(), 30), MYFONT, Brushes.Black, X1 + 185, Y1 + 90)
            e.Graphics.DrawString(row("quantidade").ToString(), MYFONT, Brushes.Black, X1 + 305, Y1 + 90)
            e.Graphics.DrawString(row("valortotalpeca").ToString(), MYFONT, Brushes.Black, X1 + 405, Y1 + 90)
    ...
    Private Sub btnImprimir_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        sql = "Select * from tbOrcamento where nrorcamento <> '' order by Codigo"
        tbOrcamento = OpenRecordset(sql)
    ...
    Private Sub btnExcluir_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnExcluir.Click
        ...
        If tbOrcamento.Rows.Count = 0 Then
            ...
            Exit Sub
        End If
        txtData.Text = tbOrcamento.Rows(0)("data").ToString
        cboClientes.Text = tbOrcamento.Rows(0)("cliente").ToString
        cboFuncionario.Text = tbOrcamento.Rows(0)("funcionario").ToString
        txtValortotalservico.Text = tbOrcamento.Rows(0)("valorservico").ToString
        txtValortotalpeca.Text = tbOrcamento.Rows(0)("valorpeca").ToString
        txtDesconto.Text = tbOrcamento.Rows(0)("desconto").ToString
        txtTotalgeral.Text = tbOrcamento.Rows(0)("totalgeral").ToString
        cbxAprovado.Text = tbOrcamento.Rows(0)("aprovado").ToString
        If MsgBox("Tem Certeza Que Deseja Excluir o Orçamento ?", MsgBoxStyle.YesNo + MsgBoxStyle.Question) = MsgBoxResult.Yes Then
            sql = "delete from tbOrcamento where nrorcamento = " & txtNumero.Text
            tbOrcamento = OpenRecordset(sql)
        End If
